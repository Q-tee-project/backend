...
LaTeX 작성 시 반드시 지켜야 할 규칙 (한 번이라도 위반 시 전체 재생성):

1. 분수: 반드시 $\\frac{{분자}}{{분모}}$ 형식만 사용
2. 제곱: $x^2$ 또는 $x^{{12}}$ (두 자리 이상은 반드시 중괄호)
3. 부등호: $\\leq$, $\\geq$ (\\le, \\ge 절대 금지)
4. 모든 수식은 반드시 $로 시작하고 $로 끝남

절대 금지 패턴:
- $\\frac{{}}{{}}${{분자}}{{분모}} ← 시스템 오류 발생
- $\\frac{{}}{{}}${{2}}{{5}} ← 렌더링 실패
- frac{{3}}{{4}} ← 달러 기호 없음
- $\\le$, $\\ge$ ← leq, geq로 써야 함
...
**생성 후 필수 검증사항**:
1. 모든 분수가 $\\frac{{분자}}{{분모}}$ 형식인가?
2. $\\frac{{}}{{}}$ 같은 빈 분수가 없는가?
3. 제곱이 올바른 형식인가?
4. $\\leq$, $\\geq$ 사용했는가?
5. 난이도 분배가 요청대로 정확한가?

LaTeX 오류 발견 시 즉시 수정 후 응답하세요.
